...
def predict():
    ...
    model = YOLO('/home/xnwu/wangyong/Code/Yolov8/runs/detect/yolov8s_train_dataset8/weights/best.pt')  # 加载自定义的模型权重作评估

   	# 评估
    imgsz=640
    metrics = model.val(imgsz=640,data='ultralytics/cfg/score_data.yaml')  # 不需要传参，这里定义的模型会自动在训练的数据集上作评估
    print("======>imsize:",imgsz)
